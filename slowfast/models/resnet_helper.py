...
from slowfast.models.hdr_helper import HDRModule
import torch
...
class ResStage(nn.Module):
    ...
    def __init__(
        ...
        self.hdr_module = hdr_frames is not None
    ...
    def _construct(
        ...
        for pathway in range(self.num_pathways):
            for i in range(self.num_blocks[pathway]):
                ...
                if hdr_frames is not None and i in hdr_blocks:
                    hdr_module = HDRModule(dim_out[pathway], hdr_frames[pathway], use_intra, use_embed)
                    self.add_module("pathway{}_hdr{}".format(pathway, i), hdr_module)
    ...
    def forward(self, inputs):
        ...
        for pathway in range(self.num_pathways):
            ...
            for i in range(self.num_blocks[pathway]):
                ...
                x = m(x)
                ori_shape = x.shape
                if hasattr(self, "pathway{}_hdr{}".format(pathway, i)):
                    hdr =  getattr(self, "pathway{}_hdr{}".format(pathway, i))
                    x = hdr(x, masks)
